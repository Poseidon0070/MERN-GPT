...
const userSignup = async(req : Request, res : Response) => { 
    try{
        ...
        res.clearCookie("auth_token", { httpOnly: true, path : '/', domain : 'master--mern-bot.netlify.app', signed:true, sameSite:"none", secure:true }) 
        let token = createToken(user._id.toString(), user.email)
        const expires = new Date()
        expires.setDate(expires.getDate() + 7)
        res.cookie('auth_token', token , { httpOnly: true, path : '/', domain : 'master--mern-bot.netlify.app', expires: expires, signed:true, sameSite:"none", secure:true })
...
const logoutUser = async(req : Request, res : Response) => {
    try{
        res.clearCookie("auth_token", { httpOnly: true, path : '/', domain : 'master--mern-bot.master--mern-bot.netlify.app', signed:true, sameSite:"none", secure:true })
